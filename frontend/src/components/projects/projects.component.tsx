...
export default function Projects(props: ProjectsComponentsProps) {
  const { type } = props;
  ...
  const hook = useGetProjectsByType(type);
  const { data } = hook({
    page,
  ...
  useEffect(() => {
    const pagesToPrefetch = getVisiblePages(page, totalPages).filter(
      (pageObject) => pageObject !== page,
    );
