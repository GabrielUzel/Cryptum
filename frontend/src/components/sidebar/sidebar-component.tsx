...
import { Sidebar, SidebarHeader, SidebarContent, SidebarGroup, SidebarFooter, SidebarProvider } from "../ui/sidebar";
  ...
  return (
    <SidebarProvider>
      <Sidebar className="!w-[250px] bg-background text-white [&_*]:bg-transparent border-r border-card">
        <SidebarHeader>
          <Button onClick={(event) => {handleCreateProjectButtonClick(event)}} className="cursor-pointer rounded-3xl !bg-primary hover:!bg-primary/60">
            Novo Projeto
          </Button>
        </SidebarHeader>
        <SidebarContent>
          ...
          <SidebarGroup>
            <SidebarLink href="/trash_bin" icon_path="trash-icon.svg" label="Lixeira" />
          </SidebarGroup>
        </SidebarContent>
        <SidebarFooter>
          {
      ...
      <CreateProjectDialog open={dialogOpen} setOpen={setDialogOpen} onCreate={handleProjectCreation} />
    </SidebarProvider>
  );
}