defmodule ColaborativeEditor.Document do
  use GenServer
  alias ColaborativeEditor.Document.Supervisor
  alias TextDelta
  alias ColaborativeEditor.DeltaConverter
  ...
  def handle_call(:get_contents, _from, state) do
    # response = Map.take(state, [:version, :content])
    response = %{
  ...
  defp name(filename), do: {:via, Registry, {ColaborativeEditor.Registry, filename}}
